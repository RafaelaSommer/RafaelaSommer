...
function generateDashboard(data){
  ...
  repos.sort((a,b)=>b.stargazerCount - a.stargazerCount).forEach(r=>{
    repoList += `
      ...
      <text x="${cardPadding + 400}" y="${repoY}" fill="#FFD93D" font-size="14">⭐ ${r.stargazerCount}</text>
      <text x="${cardPadding + 500}" y="${repoY}" fill="#8B949E" font-size="12">${r.primaryLanguage?.name || "—"}</text>`;
    repoY += 28;
  });

  y = repoY + 40; // altura final do SVG

  const svg = `
<svg width="${width}" height="${y}" xmlns="http://www.w3.org/2000/svg">
  <rect width="100%" height="100%" fill="#0D1117"/>
  <rect x="20" y="20" width="${width-40}" height="${y-40}" rx="25" fill="#161B22" stroke="#30363D"/>
