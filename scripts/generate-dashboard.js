...
function generateDashboard(data) {
  ...
  sortedLang.forEach(([lang,val]) => {
    ...
    const barWidth = percent * 5; // escala da barra
  ...
  // Repositórios
  let repoList = "";
  repos.sort((a,b)=>b.stars - a.stars).forEach(r => {
    repoList += `
      <text x="${cardPadding}" y="${y}" fill="#58A6FF" font-size="14">📦 ${r.name}</text>
      <text x="${cardPadding+350}" y="${y}" fill="#FFD93D" font-size="14">⭐ ${r.stars}</text>
      <text x="${cardPadding+450}" y="${y}" fill="#8B949E" font-size="12">${r.language}</text>
    `;
    y += 28;
  });
...
<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${cardPadding+300}" y="130" fill="#E6EDF3" font-size="16">⭐ ${stars} Stars</text>
