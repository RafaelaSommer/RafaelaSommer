...
function generateDashboard(data) {
  ...
  repos.sort((a, b) => b.stargazerCount - a.stargazerCount).forEach(r => {
    repoList += `
      <text x="${cardPadding}" y="${y}" fill="#58A6FF" font-size="14">📦 ${r.name}</text>
      <text x="${cardPadding + 350}" y="${y}" fill="#FFD93D" font-size="14">⭐ ${r.stargazerCount}</text>
      <text x="${cardPadding + 450}" y="${y}" fill="#8B949E" font-size="12">${r.primaryLanguage?.name || "—"}</text>
    `;
    y += 28;
  });
...
<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${cardPadding}" y="80" fill="#58A6FF" font-size="28" font-weight="bold">📊 Dashboard de Repositórios</text>
  <text x="${cardPadding}" y="130" fill="#E6EDF3" font-size="16">📦 ${totalProjects} Repositórios</text>
  <text x="${cardPadding + 300}" y="130" fill="#FFD93D" font-size="16">⭐ ${stars} Stars</text>
  <text x="${cardPadding}" y="170" fill="#8B949E" font-size="16">Linguagens Mais Utilizadas</text>

  ${langBars}

  <line x1="${cardPadding}" y1="${reposTitleY - 20}" x2="${width - cardPadding}" y2="${reposTitleY - 20}" stroke="#30363D"/>
  <text x="${cardPadding}" y="${reposTitleY}" fill="#8B949E" font-size="18" font-weight="bold">📂 Todos os Repositórios (${repos.length})</text>
